...
export const DEFAULT_PROTECTED_PATHS = ["src/**", "tests/**", "test/**", "**/*.test.ts", "**/*.spec.ts"];
...
export async function saveConfig(layer: ConfigLayer, change: Partial<PipowersConfig>): Promise<void> {
  ...
  if (fs.existsSync(target)) {
    try {
      ...
      log.error(`Refusing to overwrite malformed ${target}. Fix the file first.`);
      throw err;
    }
  }
...
export async function loadConfig(): Promise<LoadResult> {
  ...
  if (globalExists) {
    try {
      ...
      log.error(`Failed to parse ${globalPath}: ${err instanceof Error ? err.message : err}. Using defaults.`);
    }
  }
  ...
  if (projectExists) {
    try {
      ...
      log.error(`Failed to parse ${projectPath}: ${err instanceof Error ? err.message : err}. Using defaults.`);
    }
  }
